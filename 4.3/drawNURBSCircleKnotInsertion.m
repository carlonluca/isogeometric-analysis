...
grid on;
box on;
title('NURBS circle with 12 knots', "fontsize", 6);
xlabel('x');
ylabel('y');
...
grid on;
box on;
title('NURBS basis functions', "fontsize", 6);
s = sprintf('R_{i}^{%d}, i=0,...,%d', p, n);
xlabel('\xi');
ylabel(s);
drawNURBSBasisFuns(n, p, Xi, w);

[nBarP, barXi, barPw] = computeKnotInsertion(n, p, Xi, Pw, 0.6, 6, 0, 1);
[nBarP, barXi, barPw] = computeKnotInsertion(nBarP, p, barXi, barPw, 0.3, 4, 0, 1);
...
grid on;
box on;
title('NURBS circle with 14 knots', "fontsize", 6);
xlabel('x');
ylabel('y');
...
box on;
title('NURBS basis functions', "fontsize", 6);
s = sprintf('R_{i}^{%d}, i=0,...,%d', p, nBarP);
xlabel('\xi');
ylabel(s);
