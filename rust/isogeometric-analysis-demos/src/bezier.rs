...
use isogeometric_analysis::core::{p1, p2, p3};
...
/// 
pub fn show_bezier_curve_demo_1() {
    let cpoints = vec![
        p2(0f64, 0f64),
        p2(1f64, 1f64),
        p2(2f64, 0.5f64),
    ...
    show_bezier_curve_demo(cpoints);
}

///
/// Shows the second demo of a Bezier curve.
/// 
pub fn show_bezier_curve_demo_2() {
    let cpoints = vec![
        p3(0f64, 0f64, 0f64),
        p3(1f64, 1f64, 1f64),
        p3(2f64, 0.5f64, 0f64),
...
pub fn show_bezier_surf_demo_1() {
    let cpoints = vec![
    ...
    show_bezier_surf_demo(Array2D::from_rows(&cpoints));
}

///
/// Shows a Bezier curve with its bernstein basis polynomials.
/// 
pub fn show_bezier_curve_demo<const SIZE: usize>(cpoints: Vec<RealPoint<SIZE>>)
{
...
/// Shows a Bezier curve with its bernstein basis polynomials.
/// 
pub fn show_bezier_surf_demo(cpoints: Array2D<RealPoint3d>)
{
    let mut fg = Figure::new();
    fg.set_terminal("qt", "");
    ...
    let n = bez.degree_xi();
    let m = bez.degree_eta();
    let before = Instant::now();
    let r = RealRange { a: 0f64, b: 1f64 };
    let (_xpoints, ypoints) = Evaluator::<2, 3>::evaluate_parametric_range2d(&bez, &r, &r, &100);
    log::info!("Bezier curve computed in: {} μs", before.elapsed().as_micros());
    let (xvalues, yvalues, zvalues) = Evaluator::<2, 3>::split_coords(0, &ypoints, 1, &ypoints, 2, &ypoints);
