...
function S = computeBsplineSurfPoint(n, p, Xi, m, q, Eta, P, xi, eta_)
    xiSpan = findSpan(n, p, xi, Xi);
    etaSpan = findSpan(m, q, eta_, Eta);
    Nxi = computeBsplineBasisFuns(xiSpan, xi, p, Xi);
    Neta = computeBsplineBasisFuns(etaSpan, eta_, q, Eta);
    d = length(P(1, 1, :));
    S(d) = 0;
    for i = 1:d
        S(i) = Nxi*P(xiSpan-p+1:xiSpan+1, etaSpan-q+1:etaSpan+1, i)*Neta';
    end
